...
## 1) Hardest bug (draft, personalize before submission)
The hardest bug in this build was balancing deterministic audit logic with asynchronous AI summaries without making the user wait. The symptom was that early versions either blocked the `/api/audit` response waiting for Anthropic, or returned incomplete results where summary text could be missing and the UI looked broken. My first hypothesis was that we could simply call Anthropic inline with a short timeout and still keep acceptable latency, but that still caused inconsistent behavior under transient API failures and rate-limits.
...
## 2) A decision reversed (draft, personalize)
...
## 3) Week 2 build (draft, personalize)
...
## 4) AI tool usage (draft, personalize)
...
## 5) Self-ratings (draft, personalize)
- Discipline: 8/10
- Code quality: 8/10
- Design sense: 8/10
